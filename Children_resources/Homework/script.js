document.addEventListener('DOMContentLoaded', function() {
    ...
    // Sample homework data (in a real application, this would come from a server)
    const homeworkItems = [
        {
            title: 'Math Homework',
            description: 'Complete exercises 1-10 on page 25 of your textbook.',
            dueDate: '2023-05-15',
            downloadUrl: '#',
            primaryClass: 'Primary 3'
        },
        {
            title: 'English Essay',
        ...
        },
        {
            ...
            dueDate: '2023-05-25',
            downloadUrl: '#',
            primaryClass: 'Primary 4'
        }
    ];

    // Function to create a homework item element
    function createHomeworkItem(item) {
        const homeworkItem = document.createElement('div');
        homeworkItem.classList.add('homework-item');
        homeworkItem.innerHTML = `
            <h2>${item.title}</h2>
            <p class="primary-class">Class: ${item.primaryClass}</p>
            <p>${item.description}</p>
            <p>Due Date: ${item.dueDate}</p>
            <a href="${item.downloadUrl}" class="download-btn" download>Download</a>
        `;
        return homeworkItem;
    }
    ...
    homeworkItems.forEach(item => {
        const homeworkItemElement = createHomeworkItem(item);
        homeworkList.appendChild(homeworkItemElement);
    });
});